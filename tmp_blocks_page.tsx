...
export default function BlocksPage() {
  ...
  const ok = confirm(
    "Tout réinitialiser ? Cela efface les blocs locaux, la mémoire de l’agent et les paramètres de livre."
  );
  if (!ok) return;
  ...
  try {
    ...
    // 🧹 Supprime aussi les paramètres et aperçus du builder (draft)
    try {
      localStorage.removeItem("draft:builder:v1");
    } catch (e) {
      console.warn("Impossible de supprimer le cache du builder :", e);
    }

    // 🔁 Réinitialisation de la session agent
    try {
