      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // relance à chaque entrée dans la zone
            setVisible(false);
            setTimeout(() => setVisible(true), 100); // petit délai pour redémarrer l'animation
          }
        });
      },
      { threshold: 0.3 } // 30% visible = déclenchement
    );
...
const [generatingBook, setGeneratingBook] = useState(false);
  ...
  const t = setTimeout(() => {
    ...
    const payload = {
      ...
      titlesById,
      pointOfView: pointOfView || "first", // valeur par défaut → stable
...
async function handleGenerateBook() {
  ...
  try {
    ...
    if (!json.ok || !json.text) {
      throw new Error(json.error || "Aucune réponse générée.");
    }

    // 💾 on stocke le texte généré localement
...
<Card className="p-6 space-y-5">
  ...
  {previewText && (
    <div className="mt-6 space-y-6">
     {/* 📘 COUVERTURE + PAGE D’INTRO PREMIUM */}
<div className="relative mx-auto w-full flex flex-col items-center py-10">
  ...
  <div className="relative p-10 bg-gradient-to-br from-gray-50 to-gray-100 rounded-2xl shadow-inner border border-gray-200">
    ...
    {/* Livre + page d’intro côte à côte */}
      ...
      <div className="relative w-[350px] h-96 bg-[#fdfaf6] rounded-lg shadow-xl border border-gray-200 p-6 overflow-hidden font-serif leading-[1.8] text-gray-800">
        ...
        <div className="relative h-full">
          ...
          <p className="text-[14px] leading-relaxed mb-4">
            Ce livre racontera votre parcours unique, vos souvenirs, vos émotions.
            Chapitre après chapitre, votre histoire deviendra un héritage à transmettre.
          </p>

          <ul className="text-sm text-gray-600 list-disc pl-5 space-y-1">
            <li>Rédigé à partir de vos réponses d’interview</li>
            <li>Structure automatique en chapitres</li>
            <li>Style et ton personnalisables</li>
            <li>Disponible en version numérique et imprimée</li>
          </ul>

          {/* 🖋️ Phrase inspirante animée au scroll */}
<AnimatedQuote />
        </div>
      </div>
    </div>
  ...
  <div className="mt-4 px-6 py-2 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full shadow-sm text-gray-700 text-sm font-medium flex items-center gap-2">
    <span>🔒 Aperçu limité — La couverture et le contenu complet seront personnalisables après paiement</span>
  </div>
</div>
...
<Card className="p-6 space-y-4">
  ...
  <div className="text-center text-gray-500 py-6 animate-pulse">
    🪶 L’agent rédige votre livre… Cela peut prendre quelques instants.
  </div>
)}
